...
if __name__ == "__main__":
    ...
    '''
    Simple Step-11 Version
    # --- NEW: compute inverse-frequency class weights to fight collapse ---
    total_bins = int(np.prod(num_bins)) + (1 if args.end_token else 0)
    with torch.no_grad():
        counts = torch.zeros(total_bins, dtype=torch.long)
        for _, _, y in train_loader:                  # y: (B, T)
            y = y[:, 1:].reshape(-1)                  # align with logits (drop first token)
            y = y[(y >= 0) & (y < total_bins)]        # drop pads/invalid
            counts += torch.bincount(y, minlength=total_bins)
        weights = 1.0 / counts.clamp_min(1).float()   # inverse frequency
        weights = weights / weights.mean()            # normalize for stability

    weights = weights.to(device)
    # make criterion actually use the weights
    ...
    '''
    # Step-11 Adding
    # --- NEW: class-balanced weights (effective number of samples) + mild clipping ---
    total_bins = model.total_bins  # safer than recomputing
    with torch.no_grad():
        counts = torch.zeros(total_bins, dtype=torch.long)
        for _, _, y in train_loader:               # y: (B, T)
            y = y[:, 1:].reshape(-1)               # align with logits
            y = y[(y >= 0) & (y < total_bins)]     # drop pads/invalid
            counts += torch.bincount(y, minlength=total_bins)

        # Class-Balanced weights (Cui et al., CVPR'19)
        beta = 0.999   # 0.99~0.9999; larger => gentler reweighting
        counts_f = counts.clamp_min(1).float()
        eff_num = 1.0 - torch.pow(beta, counts_f)
        weights = (1.0 - beta) / eff_num          # smoother than 1/frequency

        # Optional: prevent ultra-rare bins from dominating
        k = 5.0                                    # cap at k×mean
        mean_w = weights.mean()
        weights = torch.clamp(weights, max=k * mean_w)

        # If you have an EOS/end_token, keep it moderate to avoid EOS hugging
        if args.end_token:
            eos_id = total_bins - 1
            weights[eos_id] = mean_w
    ...
    # register as buffer so it moves/saves with the model
    if not hasattr(target, "class_weights"):
        target.register_buffer("class_weights", weights)
    else:
        target.class_weights.copy_(weights)

    target.criterion = torch.nn.CrossEntropyLoss(
        ignore_index=target.ignore_index,
        weight=target.class_weights
    )
    # --- END NEW ---
